...
def get_AArch64_register_metadata(reg_name: str, shape_hint: str):
    # this is not fully accurate:
    # ADCLB_ZZZ_S has a ZPR, but the suffix _S implies shape: s
    # i dont want to parse based on LLVM name though as those are not 100% stable
    # Also the shape_hint might not apply to all registers, e.g. an instruction could read a FPR128.h but write a scalar FPR64
    z_v_regs = {
        "FPR16": {"prefix": "v"},
        "FPR32": {"prefix": "v"},
        "FPR64": {"prefix": "v"},
    ...
    if shape_hint:
        ...
        for val in z_v_regs.values():
            val["shape"] = shape_hint
        if reg_name in z_v_regs.keys():
            return z_v_regs[reg_name]
        # else:
        #     print("unexpected hint ", shape_hint, " on reg ", reg_name)

    metadata_map = {
        "GPR32": {"prefix": "w"},
        "GPR32sp": {"prefix": "w"},  # registers including SP
        "GPR64": {"prefix": "x"},
        "GPR64sp": {"prefix": "x"},
        "FPR8": {"prefix": "b"},
        "FPR16": {"prefix": "h"},
    ...
    # if reg_name not in metadata_map.keys():
    #     print("no metadata for: ", reg_name)
    return metadata_map.get(reg_name, {})


def get_AArch64_register_width(reg_name: str) -> int | None:
    decode_map = {
        "NZCV": -1,  # FLAGS
        "FPCR": 64,
        "X16": 64,
    ...
    if reg_name in decode_map.keys():
