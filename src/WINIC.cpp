...
std::tuple<ErrorCode, double, double> measureInSubprocess(unsigned Opcode, double Frequency,
    ...
    if (pid == 0) { // Child process
        ...
        double lower;
        double upper;
        std::tie(ec, lower, upper) = measureThroughput(Opcode, Frequency, RegInitValue);
...
void buildTPDatabase(std::vector<unsigned> Opcodes, double Frequency, long RegInitValue) {
    ...
    while (gotNewMeasurement) {
        ...
        for (unsigned opcode : Opcodes) {
            ...
            auto [EC, lowerTP, upperTP] = measureInSubprocess(opcode, Frequency, RegInitValue);
...
void buildLatDatabase(double Frequency, long RegInitValue) {
    ...
    for (auto &measurement : latencyDatabase) {
        ...
        if (measurement.type.isSymmetric()) {
            ...
            auto [EC, lat] = measureInSubprocess({measurement}, loopCount, Frequency, RegInitValue);
            measurement.ec = EC;
    ...
    for (auto &[dTypeA, measurementsA] : classifiedMeasurements) {
        ...
        auto [EC, lat] =
            measureInSubprocess({*smallestA, *smallestB}, loopCount, Frequency, RegInitValue);
        if (isError(EC)) {
            out(*ios,
        ...
        while (itA != measurementsA.end() || itB != measurementsB.end()) {
            ...
            if (opcodeBlacklist.find(mA->opcode) != opcodeBlacklist.end() ||
                ...
                mA->opcode == mB->opcode)
                continue;
            auto [EC, lat] = measureInSubprocess({*mA, *mB}, loopCount, Frequency, RegInitValue);
            if (isError(EC)) {
        ...
        for (LatMeasurement *mA : measurementsA) {
            ...
            auto [EC, lat] =
                measureInSubprocess({*mA, *smallestB}, loopCount, Frequency, RegInitValue);
            mA->ec = EC;
        ...
        for (LatMeasurement *mB : measurementsB) {
            ...
            // the debug .s file
            auto [EC, lat] =
                measureInSubprocess({*mB, *smallestA}, loopCount, Frequency, RegInitValue);
            mB->ec = EC;
